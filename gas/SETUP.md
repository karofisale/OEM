...
- **`importCostExcel(token, monthLabel, rows)`** — file kế toán gửi thật là "LNG KDNĐ...xlsx" (kiểm tra trực tiếp 2026-08-26, khác hẳn thiết kế ban đầu): **nhiều tab, mỗi tab 1 tháng** (tên tab dạng `"T07.26"`, lẫn với vài tab khác không phải dữ liệu tháng như "ZSD405_2022"/"CK"/"Ghi chú"/"Thẻ Z"). `CostImportPanel.jsx` **tự quét mọi tên tab khớp `/^T\d{1,2}\.\d{2}$/`, chọn tab THÁNG MỚI NHẤT** (không phải sheet đầu tiên, không cho chọn tay) rồi chuẩn hoá tên tab đó về đúng định dạng tab Cost (`"T7.26"` → `"T07.2026"`). Trong tab tháng đó, mỗi tháng có vài dòng tiêu đề/"TỔNG CỘNG"/"Ck" phía trên trước khi tới dòng tiêu đề thật (tìm ô "Mã") — và **cột chứa giá vốn đổi vị trí giữa các tháng** (tháng có cột "Giá ưu tiên", tháng khác chỉ có "Giá vốn" ở vị trí khác hẳn) nên đọc theo TÊN cột trong đúng dòng tiêu đề của tháng đó ("Mã"/"Tên"/ưu tiên "Giá ưu tiên" rồi mới tới "Giá vốn"), không cố định theo vị trí B/C/H như ví dụ ban đầu người dùng đưa (chỉ đúng cho tháng đó). Dòng "TỔNG CỘNG"/"Ck" bị loại bằng cách yêu cầu Mã là số nguyên ≥ 1000 (tổng dạng thập phân như "6.001628" hay chữ "Ck" đều bị loại). Upsert theo (SKU, Tháng) ở backend — tải lại đúng tháng đó lần 2 sẽ ghi đè, không nhân đôi dòng (bài học từ SOP_Plan/Debt import trước đó trong app này).
